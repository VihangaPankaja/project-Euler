...
def maxmum_sum(cur_line=1, currunt_position=1, currunt_sum=0):
    ...
    if cur_line == total_lines - 1:                                                                                     # at 2nd last line complete path and calculate sum
        sum_list.append(
            currunt_sum + value_for_cordinate(cur_line, currunt_position) + value_for_cordinate(cur_line + 1,
                                                                                                currunt_position))
        sum_list.append(
            currunt_sum + value_for_cordinate(cur_line, currunt_position) + value_for_cordinate(cur_line + 1,
                                                                                                currunt_position + 1))
        ...
        maxmum_sum(cur_line + 1, currunt_position, currunt_sum + value_for_cordinate(cur_line, currunt_position))
