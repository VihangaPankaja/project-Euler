...
for i,j in ((n, m) for n in range(11,100) for m in range(n + 1, 100) if n % 10 != 0 and m % 10 != 0):    # all possible numerators and denominators
    if list(str(i))[0] not in list(str(j)) and list(str(i))[1] not in list(str(j)):                      # clearing numbers that don't have similar digit
        continue
...
nuratr, denmtr = prod(i[0] for i in pos_prod), prod(i[1] for i in pos_prod)   # products of numerators and denominators

for i in range(nuratr, 0, -1):            # simplify the fraction
    if nuratr % i == 0 and denmtr % i == 0:
        nuratr /= i
        denmtr /= i
